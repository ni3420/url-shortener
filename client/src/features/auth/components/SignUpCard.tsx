          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            <div className="form-control w-full">
              <label className="label py-1">
                <span className="label-text text-xs font-medium opacity-80">Full Name</span>
              </label>
              <div className="relative flex items-center">
                <HiOutlineUser className="absolute left-3.5 text-base-content/50 h-5 w-5 z-10" />
                <input
                  type="text"
                  placeholder="John Doe"
                  disabled={isLoading || isSocialLoading !== null}
                  className={`input input-bordered w-full h-11 pl-11 bg-base-100 placeholder:opacity-40 focus:outline-none ${
                    errors.name ? "input-error bg-error/5" : "focus:border-primary"
                  }`}
                  {...register("name")}
                />
              </div>
            ...
            <div className="form-control w-full">
              <label className="label py-1">
                <span className="label-text text-xs font-medium opacity-80">Confirm Password</span>
              </label>
              <div className="relative flex items-center">
                <HiOutlineLockClosed className="absolute left-3.5 text-base-content/50 h-5 w-5 z-10" />
                <input
                  ...
                  disabled={isLoading || isSocialLoading !== null}
                  className={`input input-bordered w-full h-11 pl-11 pr-11 bg-base-100 placeholder:opacity-40 focus:outline-none ${
                    errors.confirmPassword ? "input-error bg-error/5" : "focus:border-primary"
                  }`}
              ...
              {errors.confirmPassword && (
                <label className="label py-1">
                  <span className="label-text-alt text-error font-medium">{errors.confirmPassword.message}</span>
                </label>
              )}
            </div>
            ...
            <button 
              type="submit" 
              className="btn btn-primary w-full h-11 min-h-[2.75rem] normal-case font-medium bg-gradient-to-r from-indigo-500 to-violet-600 border-none text-white shadow-lg shadow-indigo-500/10 active:scale-[0.99] transition-all mt-4" 
              disabled={isLoading || isSocialLoading !== null}
            >
              {isLoading ? (
                <>

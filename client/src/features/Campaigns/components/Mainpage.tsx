...
import { HiOutlineChartBar, HiOutlinePlus, HiOutlinePresentationChartLine } from "react-icons/hi2";
...
const MainPage = () => {
  const [activeTab, setActiveTab] = useState<"overview" | "analytics" | "create">("overview");

  const tabs = [
    { id: "overview" as const, name: "Overview", icon: HiOutlineChartBar },
    { id: "analytics" as const, name: "Analytics", icon: HiOutlinePresentationChartLine },
  ...
  return (
    <div className="w-full min-h-screen bg-base-100 dark:bg-zinc-950 text-base-content dark:text-zinc-50 antialiased selection:bg-primary/20 transition-colors duration-300">
      ...
      <div className=" top-0 z-30 w-max=[200] bg-base-100/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-base-300 dark:border-zinc-900 px-4 sm:px-6 lg:px-8">
        <div className="max-w-full mx-auto flex items-center justify-between h-16 gap-2">
            ...
            <div className="flex items-center w-full sm:w-auto gap-1 bg-base-200 dark:bg-zinc-900/60 p-1 rounded-xl border border-base-300  dark:border-zinc-800 shadow-inner  overflow-x-auto no-scrollbar">
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`h-4 w-4 shrink-0 ${isActive ? "text-primary dark:text-indigo-400" : "opacity-70"} hidden sm:flex`} />
                    <span>{tab.name}</span>
                  </button>
                );
              })}
          ...
          {activeTab !== "create" && (
            ...
            >
              ...
              <span className=" sm:inline">New Campaign</span>
            </button>
          )}
